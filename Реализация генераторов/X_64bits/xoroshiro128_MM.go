...
type Xoroshiro128_MM struct {
	S [2]uint64
}

func (x Xoroshiro128_MM) rotl(a uint64, b int) uint64 {
	return (a << b) | (a >> (64 - b))
}

func (x *Xoroshiro128_MM) Next() uint64 {
	var (
		s0     uint64 = x.S[0]
		s1     uint64 = x.S[1]
		result uint64 = x.rotl(s0*5, 7) * 9
	)
	s1 ^= s0
...
func (x *Xoroshiro128_MM) Long_jump() {
