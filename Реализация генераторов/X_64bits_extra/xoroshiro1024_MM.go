...
type Xoroshiro1024_MM struct {
	S [16]uint64
	P int
}

func (x Xoroshiro1024_MM) rotl(a uint64, b int) uint64 {
	return (a << b) | (a >> (64 - b))
}

func (x *Xoroshiro1024_MM) Next() uint64 {
	var q = x.P
	var s0 uint64 = x.S[(x.P+1)&15]
	(*x).P = (x.P + 1) & 15
	var s15 uint64 = x.S[q]
	var result uint64 = x.rotl(s0*5, 7) * 9
...
func (x *Xoroshiro1024_MM) Long_jump() {
